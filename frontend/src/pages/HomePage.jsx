...
// 예시 테이블 데이터 (날짜별 기능은 이후 백엔드 연동 예정)
const mockTables = [
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:space-x-6 space-y-4 md:space-y-0 mb-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">시간대 선택</label>
          <div className="flex space-x-2">
            <button
              className={`px-4 py-2 rounded ${
                mealTime === "lunch" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
              }`}
            ...
            <button
              className={`px-4 py-2 rounded ${
                mealTime === "dinner" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"
              }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTables.length === 0 ? (
          ...
          filteredTables.map((table) => (
            <TableCard key={table.id} table={table} onReserve={handleReserve} />
          ))
        )}
      </div>
    </div>
